...
| P-02 | All agent dirs have compose services | Compare apps/ dirs with docker/compose.yaml services | All match | — | Missing services |
| P-03 | Compose services have `ai.platform.agent=true` label | Grep docker/compose.yaml per agent | All labeled | — | Some missing |
...
| P-09 | Agent Card schema exists | Glob `apps/brama-core/config/agent-card.schema.json` | Exists | — | Missing |
| P-10 | All agents listed in INDEX.md | Cross-ref apps/ dirs with INDEX.md (project root) | All listed | Some missing | Most missing |
| P-11 | Langfuse service in compose | Grep docker/compose.yaml for langfuse | Present | — | Missing |
