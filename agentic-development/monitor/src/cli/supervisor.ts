...
import {
  ...
  listAllTasks,
  slugify,
  type TaskState as BaseTaskState,
  type TaskStatus,
} from "../state/task-state-v2.js";

// Extend TaskState with fields present in state.json but not in the TS interface
interface TaskState extends BaseTaskState {
  attempt?: number;
}
import { parseEventLine, type PipelineEvent } from "../state/events.js";
...
const AGENT_STALL_THRESHOLD: Record<string, number> = {
...
const log  = (m: string) => console.log(`${c.cyan}[sv ${ts()}]${c.reset} ${m}`);
const ok   = (m: string) => console.log(`${c.green}[sv ${ts()}] ✓${c.reset} ${m}`);
const warn = (m: string) => console.log(`${c.yellow}[sv ${ts()}] ⚠${c.reset} ${m}`);
const err  = (m: string) => console.log(`${c.red}[sv ${ts()}] ✗${c.reset} ${m}`);
...
function getLastEvents(taskDir: string, n = 5): PipelineEvent[] {
...
function getSecondsSinceLastActivity(taskDir: string): number {
  const now = Date.now();
...
function getAgentLogTail(taskDir: string, agent: string, lines = 20): string[] {
...
function removeStaleLock(taskDir: string): boolean {
...
async function applyFixAndRetry(
  ...
  const state = readTaskState(taskDir) as TaskState | null;
  if (state) {
    const newAttempt = (state.attempt ?? 1) + 1;
    setStateStatus(taskDir, "pending", undefined);
    // Update attempt
    const freshState = readTaskState(taskDir) as TaskState | null;
    if (freshState) {
  ...
  const updatedState = readTaskState(taskDir) as TaskState | null;
...
function analyzeFail(taskDir: string, state: TaskState): string {
  ...
  const diffSection = (header: RegExp) => {
...
export async function cmdSupervisor(args: string[]): Promise<number> {
  ...
  const opts: SupervisorOptions = {
    ...
    profile: (values.profile as string) || "standard",
    pollSec: parseInt(values.poll as string, 10) || 180,
    maxRetries: parseInt(values.retries as string, 10) || 3,
  };
  ...
  if (opts.slug) {
    ...
    if (!found) { err(`Task not found: ${opts.slug}`); return 1; }
    taskDir = found;
      ...
      case "pending": {
        ...
        if (stall.stalled) {
          ...
          if (!workersAlive()) {
            ...
            if (removed) warn("Removed stale claim lock");
          }
        }
        break;
      }
      ...
      case "completed": {
        ...
        if (summaryStatus === "FAIL") {
          ...
          if (retryCount <= opts.maxRetries) {
            ...
            const diag: Diagnosis = { category: "summary_fail", action: "retry", detail: "Summary FAIL" };
            const ok = await applyFixAndRetry(taskDir, diag, retryCount);
            if (!ok) { err("Cannot auto-fix. See fix-proposal.md"); return 1; }
          } else {
